"""Query a RAG-System using your own search index.

Based on:
https://github.com/huggingface/transformers-research-projects/blob/main/rag/use_own_knowledge_dataset.py
"""
...
def main(
    ...
    """Start the main function where everything comes together."""
    passages_path = os.path.join(rag_example_args.output_dir, "my_knowledge_dataset")
    if os.path.exists(passages_path):
        logger.info("Step 1 - Loading the dataset")
        from datasets import load_from_disk

        dataset = load_from_disk(passages_path)  # to reload the dataset
    else:
        ######################################
        logger.info("Step 1 - Creating the dataset")
        ######################################
        ...
        # More info about loading csv files in the documentation:
        # https://huggingface.co/docs/datasets/loading_datasets?highlight=csv#csv-files
    ...
    # uncomment to stop using your own code, and use fast code from meta instead.
    # import faiss
    # index = faiss.IndexHNSWFlat(index_hnsw_args.d, index_hnsw_args.m, faiss.METRIC_INNER_PRODUCT)
    # index = faiss.IndexFlatL2(index_hnsw_args.d)
    index = Index()
    ...
    # models.rag.retrieval_rag.Index
    ...
    ######################################
    logger.info("Step 4 - Processing Question")
    ######################################
...
class RagExampleArguments:
    ...
    csv_path: str = field(
        default="./data_dir/random_info.csv",
        metadata={
...
class IndexHnswArguments:
    """Storage for Hierarchical navigable small world (HNSW) index arguments."""
...
if __name__ == "__main__":
    ...
    rag_example_args, processing_args, index_hnsw_args = (
